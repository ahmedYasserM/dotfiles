...
let angle =
  (Number.parseFloat(prompt(`What is the angle you want to calculate its cos value`)) * Math.PI) / 180;
document.write(
  `<h3> cos ${(angle * 180) / Math.PI}<sup>o</sup> is ${(Math.cos(angle) * 180) / Math.PI} </h3>`
);
